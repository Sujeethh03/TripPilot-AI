...
Backend runs as a Docker container on **Render** (blueprint in `render.yaml`); the frontend is a **Vercel** project pointed at `frontend/`. Deploy the backend first — the frontend needs its URL, and the backend needs the frontend's origin for CORS.

### 1. Backend → Render

In Render: **New → Blueprint**, point it at this repo. It reads `render.yaml` and creates the web service plus a free Postgres 16 instance, wiring `DATABASE_URL` automatically.

Then set the secrets marked `sync: false` under the service's **Environment** tab:
...
| `GOOGLE_CLIENT_ID` | Google Sign-In client id (leave blank to disable Google auth) |
| `CORS_ORIGINS` | `["https://<your-app>.vercel.app"]` — JSON array, no trailing slash |

`JWT_SECRET` is generated by Render, and `CHECKPOINTER=postgres` is set in the blueprint so conversations survive the restarts the free plan imposes.

The container entrypoint runs `alembic upgrade head` before starting uvicorn, so the schema is created on first boot. Health check: `/api/v1/health`.

### 2. Frontend → Vercel

Import the repo, set **Root Directory** to `frontend`. Framework preset (Next.js), build command, and output are all detected. Set:
...
These are baked in at build time, so changing one needs a redeploy. If they're unset the build silently falls back to `localhost:8000` and the deployed app will fail to reach the API — set them before the first build.

Finally, add the Vercel URL to the backend's `CORS_ORIGINS` and, in the Google Cloud console, to the OAuth client's authorized JavaScript origins.

### Notes

- **Cold starts.** Render's free plan spins the service down after ~15 min idle; the first request then takes ~30–60s. Planning turns run over the WebSocket, which keeps the connection alive through the 30–90s generation (uvicorn pings every 20s), so only the initial wake is slow.
- **MCP servers** run in-process as stdio subprocesses inside the same container — nothing separate to deploy.
- **Leaving the free tier**: the image is plain Docker, so Railway or Fly.io is a `render.yaml` → equivalent-config swap, no code change.
